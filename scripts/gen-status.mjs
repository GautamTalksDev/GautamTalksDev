...
const stat = (x, label, value, accent) => `
  <text x="${x}" y="62" font-size="11" fill="#475569" letter-spacing="1.2">${esc(label)}</text>
...
<stop offset="0%" stop-color="#03050a"/><stop offset="100%" stop-color="#080d18"/></linearGradient>
...
<text x="34" y="34" font-size="12" fill="#334155" letter-spacing="2">LIVE SYSTEM STATUS</text>
<circle cx="196" cy="30" r="4" fill="#22c55e"><animate attributeName="opacity" values="1;0.2;1" dur="2s" repeatCount="indefinite"/></circle>
<text x="966" y="34" text-anchor="end" font-size="11" fill="#334155">rebuilt ${esc(built)}</text>
<line x1="34" y1="46" x2="966" y2="46" stroke="#152238"/>
${stat(34,  "PUBLIC REPOS",  user.public_repos,                    "#22d3ee")}
${stat(234, "LANGUAGES",     Object.keys(langs).length,            "#818cf8")}
${stat(414, "LAST PUSH",     since === null ? "—" : since + "h",   "#f472b6")}
${stat(594, "FOLLOWING",     user.following,                       "#22c55e")}
${stat(774, "STACK",         topLangs.map(([l]) => l[0]).join(""), "#f59e0b")}
<line x1="34" y1="126" x2="966" y2="126" stroke="#152238"/>
<text x="34" y="154" font-size="13" fill="#475569">last commit <tspan fill="#22d3ee">${esc(where)}</tspan></text>
<text x="34" y="176" font-size="13" fill="#94a3b8">"${esc(msg)}"</text>
<text x="966" y="176" text-anchor="end" font-size="12" fill="#334155">${topLangs.map(([l, n]) => `${l.toLowerCase()}:${n}`).join("  ")}</text>
<rect x="0.5" y="0.5" width="999" height="209" rx="14" fill="none" stroke="#152238"/>
</g></svg>`;
